class TicketTrackingServiceSync < ApplicationService
  ...
  def no_sync_required
    ...
    return true if @event.tickets.blank?
  end

  def create_event_tracking_sync_object
    @event_tracking_sync = EventTrackingSync.new
    @event_tracking_sync.event = @event
    @event_tracking_sync.status = :pending
